...
export default function GameBoyShell({ initialPage }) {
  ...
  useEffect(() => {
    ...
    if (bgAudioRef.current) {
      muted ? bgAudioRef.current.pause() : bgAudioRef.current.play().catch(() => {})
    }
  }, [muted])
  ...
  function handleActivate(idx) {
    ...
    if (idx === 0) {
      ...
      window.open(EXTERNAL.messages, '_blank', 'noopener')
    } else if (idx === 3) {
          ...
          <div className="controls">
            ...
            <div className="small-buttons">
              <div className="select-start">
                <div className="sel">SELECT</div>
                <div className="str">START</div>
              </div>
              <div className="ab">
                <div
                  className="btn a"
                  onClick={() => window.open(EXTERNAL.movies, '_blank', 'noopener')}
                  aria-label="A button"
                >
                  A
                </div>
                <div
                  className="btn b"
                  onClick={() => window.open(EXTERNAL.playlist, '_blank', 'noopener')}
                  aria-label="B button"
                >
                  B
                </div>
